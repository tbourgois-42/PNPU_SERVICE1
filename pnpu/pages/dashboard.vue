        ...
        <v-col cols="10">
          <v-list-item>
            <v-list-item-content>
                    ...
                    >
                      <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                          Workflow ID : # {{ workflow.WORKFLOW_ID }} - ID
                          Instance : #{{ workflow.ID_H_WORKFLOW }}
                        </v-list-item-subtitle>
                      </v-list-item-content>
                    </v-list-item>
                  </v-list>
              ...
              <v-list-item-subtitle>
                Workflow ID : #{{ workflowID }} - Instance : #{{ idInstanceWF }}
              </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-col>
...
export default {
  ...
  methods: {
    ...
    initialize() {
      ...
      vm.loadingData = true
      axios
        .get(`${process.env.WEB_SERVICE_WCF}/Clients/Dashboard/`, {
          params: {
            user: this.user,
        ...
        .then(function (response) {
          vm.items = response.data.GetInfoDashboardCardResult
          if (response.data.GetInfoDashboardCardResult.length > 0) {
            vm.workflowID = response.data.GetInfoDashboardCardResult[0].WORKFLOW_ID.toString() // Set WorkflowID
            vm.idInstanceWF = response.data.GetInfoDashboardCardResult[0].ID_H_WORKFLOW.toString() // Set IdInstanceWF
    ...
    async getHistoricWorkflow() {
      try {
        const response = await axios.get(
          `${process.env.WEB_SERVICE_WCF}/dashboard/workflow/historic/`,
          {
            params: {
              user: this.user,
              habilitation: this.profil,
        ...
        if (response.status === 200) {
          ...
          //this.initialize()
          this.getWorkflowName(this.idInstanceWF)
          this.getMaxStep()
        }
      } catch (error) {
        this.showSnackbar(
    ...
    /**
     * Get workflow's informations showing on dashboard.
     * @param {object} idInstanceWF - Selected instance in head of dashboard page
     */
    getWorkflowName(idInstanceWF) {
      for (let index = 0; index < this.workflows.length; index++) {
        const element = this.workflows[index]
        if (element.ID_H_WORKFLOW.toString() === idInstanceWF.toString()) {
    ...
    majDashboard(item) {
      ...
      axios
        ...
        .then(function (response) {
          vm.items = response.data.GetInfoDashboardCardByWorkflowResult
          if (vm.items.length !== 0) {
            vm.workflowID = response.data.GetInfoDashboardCardByWorkflowResult[0].WORKFLOW_ID.toString() // Set WorkflowID
            vm.idInstanceWF = response.data.GetInfoDashboardCardByWorkflowResult[0].ID_H_WORKFLOW.toString() // Set IdInstanceWF
            vm.getWorkflowName(vm.idInstanceWF)
            vm.getMaxStep()
            vm.setCardInfos()
            vm.items.forEach((client) => {
              axios
            ...
            vm.showSnackbar('error', "Aucune donnée n'a été récupéré.")
            vm.loadingData = false
          }
        })
        .catch(function (error) {
