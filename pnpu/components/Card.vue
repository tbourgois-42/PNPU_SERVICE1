<template>
  <v-hover v-slot="{ hover }">
    <v-card :elevation="hover ? 6 : 1"
            ...
            max-height="174"
            min-width="300"
            @click.prevent="filter = avancement.title">
      <div>
        ...
        <v-card-subtitle>Step {{ currentStep }} / {{ maxStep }}</v-card-subtitle>
        <v-card-subtitle>{{ clientTypolgie }}</v-card-subtitle>
        <v-chip v-if="currentStep == 10"
        ...
        <v-chip v-else-if="currentStep < 10"
