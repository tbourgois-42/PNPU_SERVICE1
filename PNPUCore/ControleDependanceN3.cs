    ...
    internal class ControleDependanceN3 : PControle, IControle
    {
        ...
        new public string MakeControl()
        {
            ...
            try
            {
                ...
                sRequete.Clear();
                sRequete.Append("SELECT DEP_CCT_TASK_ID, DEP_CCT_OBJECT_TYPE, DEP_CCT_OBJECT_ID, DEP_CCT_PARENT_OBJ_ID from PNPU_DEP_REF where NIV_DEP = '3' AND WORKFLOW_ID = " + Process.WORKFLOW_ID + " AND ID_H_WORKFLOW = " + Process.ID_INSTANCEWF);
                ...
                foreach (string sTacheCCT in lListeCCTManquants)
                {
                    ...
                    foreach (TacheCCT cCT in lTacheCCT)
                    {
                        if (cCT.CCT_TASK_ID == sTacheCCT)
                        {
                            ...
                            sFiltre.Append("'" + cCT.CCT_TASK_ID + "*" + cCT.CCT_OBJECT_ID + "*" + cCT.CCT_OBJECT_TYPE + "*" + cCT.CCT_PARENT_OBJ_ID + "'");
                        }
                    }
        ...
        private bool CommenteRegles(string sCheminMdb)
        {
            ...
            try
            {
                sRequete.Append("SELECT ID_TI, ID_ITEM, DT_START, ID_RULE, DT_START_CORR, ID_RULE_TI, SOURCE_CODE FROM M4RCH_RULES3");
                sRequete.Append(" where SOURCE_CODE NOT LIKE '%Autogenerated%'");
                dsDataSet = dmaManagerAccess.GetData(sRequete.ToString(), sCheminMdb);
                if ((dsDataSet != null) && (dsDataSet.Tables[0].Rows.Count > 0))
                {
                    using (OdbcConnection odbcConnection = new OdbcConnection(dmaManagerAccess.GetConnectionString(sCheminMdb)))
                    {
                        ...
                        foreach (DataRow drRow in dsDataSet.Tables[0].Rows)
                        {
                            ...
                            sRequete.Clear();
                            sRequete.Append("UPDATE M4RCH_RULES3 SET ");
                            sRequete.Append("SOURCE_CODE = ? ");
                            sRequete.Append("WHERE ");
                            sRequete.Append("ID_TI = '" + drRow[0].ToString() + "' AND ");
                            sRequete.Append("ID_ITEM = '" + drRow[1].ToString() + "' AND ");
                            sRequete.Append("DT_START = " + FormatDate(drRow[2].ToString()) + " AND ");
                            sRequete.Append("ID_RULE = '" + drRow[3].ToString() + "' AND ");
                            sRequete.Append("DT_START_CORR = " + FormatDate(drRow[4].ToString()) + " AND ");
                            sRequete.Append("ID_RULE_TI = '" + drRow[5].ToString() + "'");

                            odbcCommand.CommandText = sRequete.ToString();
